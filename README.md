...
- ✅ **Комплексная валидация** (3 этапа: Jitter, Keep-Alive, Heavy Load)
...
└── modules/
    ...
    ├── validator.py        # Комплексная валидация (3 этапа)
...
## 🔍 Валидация прокси (3 этапа)

Перед отправкой уведомлений каждый прокси проходит комплексную проверку:

### Этап 1: Jitter Check (Серийный пинг)
- **5 замеров** ping с интервалом 1 секунда
- **Расчёт среднего отклонения** (jitter)
- **Порог**: jitter ≤ 30% — прокси стабилен
...
Замер 4: 105 мс
Замер 5: 115 мс
Средний: 110 мс, Jitter: 7% ✅
```

### Этап 2: Keep-Alive (Удержание сессии)
- **Подключение** к прокси
- **Ожидание 15 секунд** без активности
- **Проверка** что сессия не оборвалась

**Пример:**
```
Сессия активна 15.2 сек ✅
```

### Этап 3: Heavy Load (Загрузка данных)
- **Загрузка** последних сообщений из канала
- **Расчёт скорости** передачи данных
- **Порог**: ≥ 50 Кбит/с

**Пример:**
```
Загружено 250 KB за 3.5 сек
Скорость: 571 Кбит/с ✅
```

### Итоговый score:
- Jitter Check: 40 баллов
- Keep-Alive: 30 баллов
- Heavy Load: 30 баллов
- **Минимум**: 70 баллов для валидности
